import pandas as pd
import numpy as np

np.random.seed(11)
#test

import tensorflow as tf
from tensorflow.keras.preprocessing.sequence import pad_sequences
from tensorflow.keras.preprocessing.text import Tokenizer



def read_dataset(train_path, test_path, max_words):
    train = pd.read_json(train_path)
    test = pd.read_json(test_path)

    train_1 = train[train["has_spoiler"] == True]
    train_0 = train[train["has_spoiler"] == False]
    train = pd.concat([train_1, train_0[:100000]], axis=0)
    reviews_data = train["review_sentences"]
    labels_train = train["has_spoiler"]
    reviews_data = [' '.join([sentence for _, sentence in review]) for review in reviews_data]


    test_1 = test[test["has_spoiler"] == True]
    test_0 = test[test["has_spoiler"] == False]
    test = pd.concat([test_1, test_0[:25000]], axis=0)
    test_data = test["review_sentences"]
    labels_test = test["has_spoiler"]
    test_data = [' '.join([sentence for _, sentence in review]) for review in test_data]

    tokenizer = Tokenizer(num_words=max_words)
    tokenizer.fit_on_texts(reviews_data)
    sequences = tokenizer.texts_to_sequences(reviews_data)

    # Padding
    max_sequence_length = max(len(seq) for seq in sequences)
    padded_train = pad_sequences(sequences, maxlen=max_sequence_length)
    sequences_test = tokenizer.texts_to_sequences(test_data)
    padded_test = pad_sequences(sequences_test, maxlen=max_sequence_length)

    return padded_train, labels_train, padded_test, labels_test, tokenizer, max_sequence_length



def embeddings(embeddings_path, embedding_dim, tokenizer, max_words):
    embeddings_index = {}

    with open(embeddings_path, 'r', encoding='utf-8') as file:
        for line in file:
            values = line.split()
            word = values[0]
            coefs = np.asarray(values[1:], dtype='float32')
            embeddings_index[word] = coefs

    embedding_matrix = np.zeros((max_words, embedding_dim))

    for word, i in tokenizer.word_index.items():
        if i < max_words:
            embedding_vector = embeddings_index.get(word)
            if embedding_vector is not None:
                embedding_matrix[i] = embedding_vector


    return embedding_matrix




